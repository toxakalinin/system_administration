...
async def hard_sysinfo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif not query or not query.message:
        return await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="Ошибка: действие не может быть выполнено."
    )
...
async def run_security_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        def perform_scan():
            try:
                ...
                lynis_result = subprocess.run(
                    ["sudo", "lynis", "audit", "system"], capture_output=True, text=True
                ).stdout

                # Run chkrootkit
                chkrootkit_result = subprocess.run(
                    ["sudo", "chkrootkit"], capture_output=True, text=True
                ).stdout

                # Run Rkhunter
                rkhunter_result = subprocess.run(
                    ["sudo", "rkhunter", "--check", "--skip-keypress"], capture_output=True, text=True
                ).stdout
                ...
                return f"Error during scan execution: {scan_error}"
        ...
        if len(scan_result) > 4000:
            file_path = "security_scan_report.txt"
        ...
        logging.error("Error in run_security_scan: %s", e)
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
...
async def server_health_check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    default_services = ["nginx", "fail2ban", "firewalld", "ssh"]
...
async def perform_security_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logging.error("Error in perform_security_scan: %s", e)
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="An error occurred during the security scan process."
        )
...
async def handle_long_output(output, filename, update, context, max_length=4096):
    if len(output) <= max_length:
        ...
        await context.bot.send_document(
            chat_id=update.effective_chat.id, document=temp_file_path,
            caption="Output too long, see attached file."
        )
...
async def generate_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        report = {
            "CPU Usage": f"{await get_cpu_usage()}%",
            "Memory Usage": await get_memory_usage(),
            "Disk Usage": await get_disk_usage(),
        }
        ...
        await context.bot.send_message(
            chat_id=update.effective_chat.id, text=report_text, parse_mode="HTML"
        )
    except Exception as e:
        logging.error("Error generating system report: %s", e)
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
...
async def monitor(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logging.error("Error in monitor: %s", e)
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text=f"Error monitoring system: {e}"
        )
...
async def run_nmap_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logging.error("Error running nmap scan: %s", e)

    await handle_long_output(result, "nmap_result.txt", update, context)

async def run_nikto_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="Please specify a target for the scan (e.g., /nikto <target>)."
        )
        return
    ...
    try:
        ...
        logging.error("Error running nikto scan: %s", e)

    await handle_long_output(result, "nikto_result.txt", update, context)
